...
- **Сканер** — на реальном устройстве: камера, звук/вибрация при сканировании, страницы приёмки/упаковки и отдельная страница сканера. На странице сканера действуют throttle (не чаще одного скана в 0,8 с) и debounce по одному штрихкоду (15 с); ввод нормализуется (trim). После успешного скана зелёная подсветка области сканера держится несколько секунд (оверлей поверх видео); частота сканирования кадров снижена (5 fps). Мигающие элементы рамки библиотеки скрыты через CSS (`#qr-shaded-region > div`), вместо них используется статичная рамка. Ошибки камеры показываются только при реальных проблемах (доступ, устройство); сообщения «ничего не найдено в кадре» не выводятся и не вызывают мелькания интерфейса. Каждое сканирование (вызов validate barcode) записывается в таблицу `barcode_scan_log` (пользователь, компания, заявка, результат).
...
- **Безопасность и память:** SSH (отключение паролей), файрвол ufw (allow 22, 80, 443; default deny), лимиты памяти контейнеров и сборка фронта без OOM — см. [docs/OOM_AND_HARDENING.md](OOM_AND_HARDENING.md).
