...
| **CompanyAPIKeys** | company_api_keys.py | API-ключи WB/Ozon компании (хранятся зашифрованно) |
| **Destination** | destination.py | Адреса доставки (склады, маркетплейсы) |
| **Product** | product.py | Товар (name, barcode, stock_quantity, defect_quantity, company_id); ProductPhoto — фото брака |
...
| **FBOSupply**, **FBOSupplyBox**, **FBOSupplyItem** | fbo_supply.py | FBO-поставки WB/Ozon: поставка, короба, позиции |
...
Поля дат/времени в большинстве таблиц хранятся в UTC без timezone (naive). Исключения: `document_chunks.created_at`, `ai_settings.updated_at` — с timezone (TIMESTAMPTZ).
