...
describe("apiClient", () => {
  ...
  it("retries request after 401 when initData available and refresh succeeds", async () => {
    localStorage.setItem("birka_session_token", "old-token");
    const telegramWindow = window as Window & {
      Telegram?: { WebApp?: { initData?: string; showAlert?: (msg: string) => void } };
    };
    telegramWindow.Telegram = { WebApp: { initData: "valid-init-data", showAlert: vi.fn() } };
    ...
    const fetchMock = vi.spyOn(global, "fetch")
      ...
      .mockResolvedValueOnce({
        // refresh /auth/telegram
        ok: true,
        json: async () => ({ session_token: "new-token" }),
      } as Response)
      .mockResolvedValueOnce({
    ...
    const result = await apiClient.api<{ id: number }>("/companies");
    expect(result).toEqual({ id: 1 });
    expect(fetchMock).toHaveBeenCalledTimes(3);
    expect(localStorage.getItem("birka_session_token")).toBe("new-token");
  });
  ...
  it("clears token and does not retry when no initData on 401", async () => {
    ...
    telegramWindow.Telegram = { WebApp: { initData: undefined, showAlert: vi.fn() } };
